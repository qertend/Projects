...
function checkGrid(lastGrid) {
        //if grid to the left
        console.log(availableGrids);
        if (lastGrid[0]-1 >= 0 && !vWalls[lastGrid[0]][lastGrid[1]] && !availableGrids.includes([lastGrid[0]-1, lastGrid[1]])) {
            availableGrids.push([lastGrid[0]-1, lastGrid[1]]);
            checkGrid([lastGrid[0]-1, lastGrid[1]]);
        }
        //if grid to the right
        if (lastGrid[0]+1 <= grid && !vWalls[lastGrid[0]+1][lastGrid[1]] && !availableGrids.includes([lastGrid[0]+1, lastGrid[1]])) {
            availableGrids.push([lastGrid[0]+1, lastGrid[1]]);
            checkGrid([lastGrid[0]+1, lastGrid[1]]);
        }
        //if grid above
        if (lastGrid[1]-1 >= 0 && !hWalls[lastGrid[0]][lastGrid[1]] && !availableGrids.includes([lastGrid[0], lastGrid[1]]-1)) {
            availableGrids.push([lastGrid[0], lastGrid[1]-1]);
            checkGrid([lastGrid[0], lastGrid[1]-1]);
        }
        //if grid below
        if (lastGrid[1]+1 >= 0 && !hWalls[lastGrid[0]][lastGrid[1]+1] && !availableGrids.includes([lastGrid[0], lastGrid[1]]+1)) {
            availableGrids.push([lastGrid[0], lastGrid[1]+1]);
            checkGrid([lastGrid[0], lastGrid[1]+1]);
        }
}
...
function generateLabyrinth() {
    ...
    ctxLab.stroke();
    checkGrid([randInt(0, grid), randInt(0, grid)]);
    while (availableGrids.length < 3) {
        checkGrid([randInt(0, grid), randInt(0, grid)]);
    }
}
